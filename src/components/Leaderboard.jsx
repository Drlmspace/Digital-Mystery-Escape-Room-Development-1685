...
const { FiTrophy, FiClock, FiUsers, FiTarget, FiHelpCircle, FiHome, FiRefreshCw, FiCalendar, FiFilter, FiAward, FiStar, FiMedal } = FiIcons;
...
export default function Leaderboard() {
  ...
  const getPerformanceRating = (team) => {
    ...
    const timeScore = team.game_state === 'completed' ? 
      Math.max(0, 100 - (team.total_time_seconds / 3600) * 100) : 0;
  ...
  const getFilteredStats = () => {
    ...
    const avgTime = completed.length > 0 ? 
      completed.reduce((acc, team) => acc + team.total_time_seconds, 0) / completed.length : 0;
    const bestTime = completed.length > 0 ? 
      Math.min(...completed.map(team => team.total_time_seconds)) : 0;
              ...
              {leaderboardData.map((team, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`bg-mystery-800 rounded-lg p-6 border-l-4 ${
                      index === 0 ? 'border-gold-400' :
