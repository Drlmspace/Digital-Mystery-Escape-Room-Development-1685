...
const {FiSettings,FiUsers,FiClock,FiVolumeX,FiVolume2,FiHome,FiDownload,FiUpload,FiMusic,FiPlay,FiPause,FiTrash2,FiCheck,FiMic,FiHeadphones,FiRefreshCw,FiDatabase,FiHelpCircle,FiKey,FiEye,FiEyeOff,FiSearch,FiFilter,FiTrophy,FiLogOut,FiShield,FiVideo,FiImage,FiSave}=FiIcons;
...
{key: 'ambient',label: 'Ambient Music',description: 'Background music for exploration and puzzle solving',icon: '🎵'},
{key: 'tension',label: 'Tension Music',description: 'Dramatic music for intense moments and discoveries',icon: '🎭'},
{key: 'success',label: 'Success Music',description: 'Victory music for puzzle completion and achievements',icon: '🏆'},
{key: 'background',label: 'Background Music',description: 'General background music for menu and transitions',icon: '🎶'}
];
...
icon: '📖',
messages: [
{key: 'stage1Intro',label: 'Stage 1 - The Locked Office',description: 'Opening narration for the first stage'},
{key: 'stage2Intro',label: 'Stage 2 - Digital Forensics Lab',description: 'Introduction to the digital investigation'},
{key: 'stage3Intro',label: 'Stage 3 - The Restricted Archives',description: 'Narration for the archives exploration'},
{key: 'stage4Intro',label: 'Stage 4 - Security System Investigation',description: 'Introduction to security footage analysis'},
{key: 'stage5Intro',label: 'Stage 5 - The Exhibition Hall',description: 'Narration for the art authentication stage'},
{key: 'stage6Intro',label: 'Stage 6 - The Final Confrontation',description: 'Final stage dramatic introduction'}
]
},
{
...
icon: '🎭',
messages: [
{key: 'voiceMessage',label: 'Dr. Blackwood\'s Voice Message',description: 'Hidden voice message with cipher clues'},
{key: 'drBlackwoodRecording',label: 'Dr. Blackwood\'s Recording',description: 'Audio recording found in the archives'},
{key: 'curatorXMessage',label: 'Curator X Message',description: 'Mysterious message from the anonymous contact'},
{key: 'finalMessage',label: 'Final Message',description: 'Dr. Blackwood\'s final message to players'}
]
},
{
...
icon: '🔊',
messages: [
{key: 'hintAudio',label: 'Hint Notification',description: 'Sound played when a hint is used'},
{key: 'successSound',label: 'Success Sound',description: 'Sound for puzzle completion'},
{key: 'errorSound',label: 'Error Sound',description: 'Sound for incorrect attempts'},
{key: 'puzzleComplete',label: 'Puzzle Complete',description: 'Sound when all puzzles in a stage are solved'}
]
}
];
...
setAudioValidation(prev=> ({
...prev,
[category]: {valid: false,message: 'Failed to load audio file'}
}));
setIsTestingAudio(prev=> ({...prev,[category]: false}));
}
};
...
...prev,
[messageKey]: {valid: false,message: 'Failed to load audio file'}
}));
setIsTestingMessage(prev=> ({...prev,[messageKey]: false}));
}
};
...
const saveMusicUrls=async ()=> {
try {
const validUrls=Object.entries(musicUrls).filter(([_,url])=> {
const validation=validateAudioUrl(url);
...
// Save to database
for (const [category,url] of validUrls) {
await dbHelpers.upsertAudioAsset({
asset_key: category,
asset_url: url,
asset_type: 'music',
...
// Also save to localStorage as backup
localStorage.setItem('customMusicUrls',JSON.stringify(musicUrls));
alert(`Saved ${validUrls.length} music URLs successfully to database!`);
} else {
alert('Please add at least one valid music URL');
}
} catch (error) {
console.error('Failed to save music URLs:',error);
alert('Failed to save music URLs to database. Please try again.');
}
};

const saveAudioMessages=async ()=> {
try {
const validMessages=Object.entries(audioMessages).filter(([_,url])=> {
const validation=validateAudioUrl(url);
return validation.valid;
});
...
// Save to database
for (const [messageKey,url] of validMessages) {
await dbHelpers.upsertAudioAsset({
asset_key: messageKey,
asset_url: url,
asset_type: 'narration',
...
// Also save to localStorage as backup
localStorage.setItem('customAudioMessages',JSON.stringify(audioMessages));
alert(`Saved ${validMessages.length} audio messages successfully to database!`);
} else {
alert('Please add at least one valid audio message URL');
}
} catch (error) {
console.error('Failed to save audio messages:',error);
alert('Failed to save audio messages to database. Please try again.');
}
};
...
<button
onClick={saveMusicUrls}
className="px-8 py-3 gold-gradient text-mystery-900 font-semibold rounded-lg hover:opacity-90 transition-opacity"
>
Save Music Configuration to Database
</button>
</div>
</div>
</motion.div>
...
<div className="mt-8 flex justify-center">
<button
onClick={saveAudioMessages}
className="px-8 py-3 gold-gradient text-mystery-900 font-semibold rounded-lg hover:opacity-90 transition-opacity"
>
Save All Audio Messages to Database
</button>
</div>
</div>
</motion.div>
